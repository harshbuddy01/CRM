...
export default function AdminCampaignPage() {
  ...
  useEffect(() => {
    ...
    // Wait until user store hydrates
    if (user) {
      if (user.role !== 'admin') {
  ...
  const handleSendCampaign = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'SMTP Server failed to route the campaign.');
    } finally {
      setSending(false);
    }
  };
  ...
  if (!authorized) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white">
        <Lock className="w-12 h-12 text-rose-500 animate-pulse mb-4" />
        <p className="text-lg font-semibold">Verifying Admin Credentials...</p>
        <p className="text-xs text-slate-400 mt-2">Checking session variables...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between border-b pb-6">
          ...
          <Badge className="bg-purple-600 hover:bg-purple-700 py-1 px-3">
            👑 Admin Console
          </Badge>
        </div>
        ...
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2">
            <Mail className="w-8 h-8 text-primary" /> Standalone Email Campaigner
          </h1>
          <p className="text-muted-foreground mt-1">
            Broadcast custom marketing announcements or transaction alerts to clients using your domain SMTP credentials.
          </p>
        </div>
