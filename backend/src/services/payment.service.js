...
const recordManualPayment = async (data, userId) => {
  ...
  // Auto-send WhatsApp receipt confirmation (non-blocking)
  sendPaymentReceiptWhatsapp(payment.id).catch(err => console.error('[WhatsApp Payment Receipt Error]:', err.message));
...
const handleWebhook = async (body, rawBody, signature) => {
  ...
  if (body.event === 'payment.captured' || body.event === 'payment_link.paid') {
    ...
    if (localPaymentId) {
      ...
      sendPaymentReceiptWhatsapp(localPaymentId).catch(err => console.error('[WhatsApp Payment Receipt Error]:', err.message));
    } else {
      ...
      sendPaymentReceiptWhatsapp(newPayment.id).catch(err => console.error('[WhatsApp Payment Receipt Error]:', err.message));
    }
...
const sendPaymentReceiptWhatsapp = async (paymentId) => {
  ...
  try {
    ...
    if (!payment || !payment.queryId) return;

    // Get confirmed proposal to calculate balance
    const proposal = await prisma.proposal.findFirst({
      where: { queryId: payment.queryId, status: 'confirmed', deletedAt: null }
    });
    if (!proposal) return; // No confirmed proposal, so no balance to calculate yet

    const verifiedPayments = await prisma.payment.findMany({
      where: { queryId: payment.queryId, status: { in: ['verified', 'banked'] }, deletedAt: null }
    });
    const totalPaid = verifiedPayments.reduce((sum, p) => sum + Number(p.amount), 0);
    const balanceDue = Math.max(0, Number(proposal.sellingPrice) - totalPaid);

    const clientPhone = payment.query.phone;
    ...
    if (config.whatsapp.mode === 'api') {
      const components = [
        {
          type: 'body',
          parameters: [
            { type: 'text', text: payment.query.name || 'Customer' },
            { type: 'text', text: `₹${Number(payment.amount).toLocaleString('en-IN')}` },
            { type: 'text', text: payment.mode.toUpperCase() },
            { type: 'text', text: `₹${Number(balanceDue).toLocaleString('en-IN')}` }
          ]
        }
      ];

      await queueService.enqueueWhatsappJob(payment.queryId, normalizedPhone, 'client_payment_received', components);
    }
  } catch (err) {
    console.error('[Payment Service] Failed to send WhatsApp receipt:', err.message);
  }
};
