    ...
    <head>
      ...
      <style>
        * { box-sizing: border-box; }
        ...
        body { margin: 0; padding: 0; width: 210mm; min-height: 297mm; font-family: 'Inter', sans-serif; font-size: 9px; color: #2d3748; background: #ffffff; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        .main-content-wrapper { padding: 25px; }
        .top-banner-container { width: 100%; height: 120px; overflow: hidden; margin-bottom: 15px; }
        ...
        .details-card { border: 1px solid #e2e8f0; border-radius: 5px; background: #ffffff; }
        .details-card-header { background: #0f3d2f; color: #ffffff; font-family: 'Cinzel', serif; font-size: 9px; font-weight: 700; padding: 5px 8px; text-transform: uppercase; }
        .details-card-body { padding: 8px; font-size: 8px; }
        .info-row { display: flex; align-items: center; gap: 5px; margin-bottom: 4px; }
        .middle-block { display: grid; grid-template-columns: 1fr 2fr 1fr; gap: 10px; margin-bottom: 10px; }
        .middle-card { border: 1px solid #e2e8f0; border-radius: 5px; }
        .middle-card-header { background: #0f3d2f; color: #ffffff; font-family: 'Cinzel', serif; font-size: 9px; font-weight: 700; padding: 5px 8px; text-transform: uppercase; }
        .middle-card-body { padding: 8px; font-size: 8px; }
        .summary-table { width: 100%; border-collapse: collapse; margin-top: 5px; }
        .summary-table th { background: #f7fafc; font-size: 7.5px; padding: 4px; text-transform: uppercase; border-bottom: 1px solid #e2e8f0; }
        .summary-table td { padding: 5px 4px; border-bottom: 1px solid #edf2f7; font-size: 8px; }
        .footer-cards-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; padding: 0 25px; margin-bottom: 20px; }
        .footer-card { border: 1px solid #e2e8f0; padding: 8px; border-radius: 5px; min-height: 100px; }
        .footer-card-title { font-family: 'Cinzel', serif; font-weight: 700; border-bottom: 1px solid #edf2f7; margin-bottom: 5px; padding-bottom: 2px; }
        .feedback-qr-img { width: 60px; height: 60px; }
        .bottom-footer-bar { background: #0f3d2f; color: #ffffff; padding: 10px 25px; display: flex; justify-content: space-between; align-items: center; font-size: 8px; }
        .bottom-logo-img { height: 25px; }
        .amount-highlight { font-weight: 700; }
      </style>
    </head>
    <body>
      <div class="top-banner-container">
        <img src="${invoiceHeaderBannerUrl}" class="top-banner-img" alt="Header" />
      </div>
      <div class="main-content-wrapper">
        <div class="details-grid">
          <div class="details-card">
            <div class="details-card-header">Invoice Details</div>
            <div class="details-card-body">
              <div class="info-row">Invoice No: <b>${invoiceNumber}</b></div>
              <div class="info-row">Ref ID: <b>${referenceId}</b></div>
              <div class="info-row">Inv Date: <b>${invoiceDate}</b></div>
              <div class="info-row">Due Date: <b style="color:red">${dueDate}</b></div>
            </div>
          </div>
          <div class="details-card">
            <div class="details-card-header">Company Details</div>
            <div class="details-card-body">
        ...
        <div class="middle-block">
          <div class="middle-card">
            <div class="middle-card-header">Bill To</div>
          ...
          <div class="middle-card">
            <div class="middle-card-header">Invoice Summary</div>
            <table class="summary-table">
              <tr><th>Description</th><th>Qty</th><th>Rate</th><th>Amount</th></tr>
              <tr><td>${descriptionText}</td><td>1</td><td>${formatNumberDec(subtotal)}</td><td class="amount-highlight">${formatNumberDec(subtotal)}</td></tr>
              <tr><td colspan="3" style="text-align:right">Total</td><td>${formatCurrencyDec(totalAmount)}</td></tr>
            </table>
            <div style="padding: 5px; font-size: 7px;">Amount in words: <b>${amountInWordsText}</b></div>
          </div>
          <div class="middle-card">
            <div class="middle-card-header">Payment</div>
            <div class="middle-card-body">
              <div>Total: ${formatCurrencyDec(totalAmount)}</div>
              <div>Paid: ${formatCurrencyDec(totalReceived)}</div>
              <div style="color:red">Due: ${formatCurrencyDec(balanceDue)}</div>
            </div>
          </div>
        </div>
      </div>
      <div class="footer-cards-grid">
        <div class="footer-card">
          ...
          <div style="font-size: 7px;">This is a system generated invoice. Please carry this document during your travel.</div>
        </div>
        <div class="footer-card" style="display:flex; justify-content:space-between;">
           <div>
            <div class="footer-card-title">Feedback</div>
            <div style="font-size: 7px;">Scan to review us on Google.</div>
           </div>
           <img src="${invoiceQrCodeUrl}" class="feedback-qr-img" />
        </div>
        <div class="footer-card">
          <div class="footer-card-title">Journey Highlights</div>
          <div style="font-size: 7px;">${durationText} | ${capitalizedDest}</div>
        </div>
      </div>
      <div class="bottom-footer-bar">
        <div>${companyName}</div>
        <div>${companyWebsite} | ${companyPhone}</div>
        ${companyLogoUrl ? `<img src="${companyLogoUrl}" class="bottom-logo-img" />` : ''}
      </div>
    </body>
    </html>
  `;
